...
export default {
  ...
  components: {
    TableListOfEmployees: () => import('@/components/tables/table-list-of-employees/TableListOfEmployees')
  },
  created () {
  ...
  methods: {
    ...
    async onSubmit () {
      try {
        ...
        if (this.isEditForm) {
          this.$swal(configMessage.confirm_edit).then(
            async ({ isConfirmed }) => {
              if (isConfirmed) {
                try {
                  await this.$store.dispatch('employee/updateAEmployee', this.form)
        ...
        // save
        // const { data } = await this.$store.dispatch('employee/createAEmployee', this.form)
        await this.$store.dispatch('employee/createAEmployee', this.form)
        await this.$swal(configMessage.success)
        this.onReset()
        this.$refs.TableListOfEmployees.refreshData()
      } catch (e) {
      ...
      await this.$swal(configMessage.confirm_edit).then(
        async ({ isConfirmed }) => {
          if (isConfirmed) {
            ...
            const { id } = employee
            try {
              const { data } = await this.$store.dispatch('employee/getAEmployee', id)
              this.$nextTick(() => {
